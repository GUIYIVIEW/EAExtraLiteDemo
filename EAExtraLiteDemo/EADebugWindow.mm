...
-(void)tick
{
    //找到当前 正在显示的 viewController ,调用其刷新，这里与自己程序的框架有关，
    //根据实际情况，自行修改如何 找到正在显示的 viewController
    if ( [[NSUserDefaults standardUserDefaults] boolForKey:@"debugSkin"] )
    {
        ...
        if( [rootVC isKindOfClass:[UINavigationController class]])
        {
            ...
            [[((UINavigationController*)rootVC) topViewController] performSelector:NSSelectorFromString(@"freshSkin")];
        }
        else
        {
